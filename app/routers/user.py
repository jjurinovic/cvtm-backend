...
async def create_user(req: UserCreate, db: Session = Depends(database.get_db), current_user: User = Depends(get_current_user)):
    return await user.create_user(req, db, current_user)


@router.put('/', response_model=User)
def update_user(req: User, db: Session = Depends(database.get_db), current_user: User = Depends(get_current_user)):
    return user.update_user(req, db, current_user)


@router.delete('/{id}', dependencies=[Depends(roles.RoleChecker(roles.Role.ADMIN))])
def delete_user(id: int, db: Session = Depends(database.get_db), current_user: UserWithCompany = Depends(get_current_user)):
    return user.delete_user(id, db, current_user)


@router.delete('/{id}/soft', dependencies=[Depends(roles.RoleChecker(roles.Role.ADMIN))])
def soft_delete_user(id: int, db: Session = Depends(database.get_db), current_user: UserWithCompany = Depends(get_current_user)):
...
def change_user_status(id: int, db: Session = Depends(database.get_db), current_user: UserWithCompany = Depends(get_current_user)):
...
def restore_user(id: int, db: Session = Depends(database.get_db), current_user: UserWithCompany = Depends(get_current_user)):
...
def get_user(id: int, db: Session = Depends(database.get_db), current_user: User = Depends(get_current_user)):
...
def get_all_users(company_id: int, page_params: PageParams = Depends(PageParams), db: Session = Depends(database.get_db), current_user: User = Depends(get_current_user)):
...
def change_password(req: PasswordChange, db: Session = Depends(database.get_db), current_user: UserCreate = Depends(get_current_user)):
